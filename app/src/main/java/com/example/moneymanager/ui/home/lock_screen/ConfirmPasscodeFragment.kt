...
class ConfirmPasscodeFragment : Fragment() {
    ...
    private fun savePasscode() {
        ...
        if (passcode.length == 6) {
            activity?.getSharedPreferences("passcode_prefs", Context.MODE_PRIVATE)?.edit()
                ?.putString(PASSCODE_PREF_KEY, passcode)?.apply()
            Toast.makeText(context, "Passcode saved", Toast.LENGTH_SHORT).show()
        } else {
            Toast.makeText(context, "Please enter a 6-digit passcode", Toast.LENGTH_SHORT).show()
        }
    }
